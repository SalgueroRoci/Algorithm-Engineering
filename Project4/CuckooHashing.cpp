...
bool place_in_hash_tables(char *s) {
	...
	size_t pos;
	int index;
	//store the strings, use temp is movement is needed
	char temp_s[255], temp[255];

	//copy the string to first temp-s
	strcpy(temp_s, s);
	...
	while ((!placed) && (counter < 2 * tablesize)) {
		...
		if (strcmp(t[pos][index], "") == 0) {
			...
			// Counter keeps track of number of switching between tables
			//first should be counter == 0 and index == 0 so switch index = 1
			if (counter % 2 == 0)
				index = 1;
			else
				index = 0;
...
size_t f(char *s, size_t index) {
	...
	if (index == 0) {
		...
		for (i = 1; i < len; i++)
		{
			//start from reverse order than do the += key[keyseize - i - 1] * 37^i
			temp = s[len - i - 1];
			po *= 37;
