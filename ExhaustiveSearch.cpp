...
{
	for (int i = 0; i < n; i++) {
		cout << "Point [" << i << "] = (" << P[seq[i]].x << " , " << P[seq[i]].y << ") , ";
	}
	cout << "Point [" << n << "] = (" << P[seq[0]].x << " , " << P[seq[0]].y << "), ";
}
...
// function to generate the permutation of indices of the list of points
{
	...
	if (n == 1) {
		...
		float dist = 0;
		for (int i = 0; i < n; i++) {
			dist = (P[A[i]].x - P[A[i + 1]].x)*(P[A[i]].x - P[A[i + 1]].x) + (P[A[i]].y - P[A[i + 1]].y)*(P[A[i]].y - P[A[i + 1]].y);
			dist = sqrt(dist);
			total += dist; 
		}
		//add the edge of starting A and ending point A to complete a cycle 
		dist = (P[A[0]].x - P[A[n-1]].x)*(P[A[0]].x - P[A[n-1]].x) + (P[A[0]].y - P[A[n-1]].y)*(P[A[0]].y - P[A[n-1]].y);
		dist = sqrt(dist);
		total += dist; 

		//found a shorter hamiltonian cycle 
		if (total < bestDist) {
			//update bestDist 
			bestDist = total; 

			//add A indexes values to bestSet found a better set~
			for (int i = 0; i < n; i++) {
				bestSet[i] = A[i]; 
			}
		}
